...
func createGroup() *gmystcache.Group {
	return gmystcache.NewGroup("scores", 2<<10, gmystcache.GetterFunc(
...
func startCacheServer(addr string, addrs []string, gee *gmystcache.Group) {
	peers := gmystcache.NewHTTPPool(addr)
	peers.Set(addrs...)
...
func startAPIServer(apiAddr string, gee *gmystcache.Group) {
...
func main() {
	var port int
	var api bool
	flag.IntVar(&port, "port", 8001, "gmystcache server port")
